...
Instructions = Dict[str, Tuple[Maneuver, Maneuver]]


class Computer:

    def __init__(self, instructions: Instructions, starting_state: str):
...
def parse_single_instruction(lines_iterator):  # this could be better...
...
def parse_instructions(lines_iterator: Iterator[str]) -> Instructions:
    ...
    try:
        while True:
            state_letter, new_instrution = parse_single_instruction(lines_iterator)
            instructions_out[state_letter] = new_instrution
...
if __name__ == '__main__':
    ...
    instructions = parse_instructions(raw_lines_iterator)
    computer = Computer(instructions, starting_state)
