...
def convert_picture_to_blocks(unified_grid):
    chop_by = next(i for i in (2, 3) if len(unified_grid) % i == 0)
    num_blocks = len(unified_grid) // chop_by
    blocks_grid = shared.initialize_2d_array(num_blocks, num_blocks, '.')
    for i in range(num_blocks):
        for j in range(num_blocks):
            cur_block_in = tuple(r[chop_by * j: chop_by * j + chop_by] for r in unified_grid[i * chop_by:chop_by * i + chop_by])
            blocks_grid[i][j] = cur_block_in
    return blocks_grid


def perform_iteration(p):
    blocks_grid = convert_picture_to_blocks(p)
    for i, blocks_row in enumerate(blocks_grid):
        for j, cur_block_in in enumerate(blocks_row):
